...
export function TopicPermissionForm({
  onSubmit,
  isLoading,
  onCancel,
  apiClient,
}: TopicPermissionFormProps) {
  const { t } = useTranslation();
  const { data: vhosts, isPending } = useVhosts(apiClient);

  const form = useForm<TopicPermissionBody & { vhost: string }>({
    resolver: zodResolver(
      topicPermissionBodySchema.extend({ vhost: z.string().min(1, "Virtual host is required") }),
    ),
    defaultValues: {
      vhost: "",
      exchange: "",
  ...
  return (
    <form onSubmit={handleSubmit((data) => onSubmit(data.vhost, data))} className="space-y-4">
      <div className="space-y-2">
        <Label htmlFor="topic-permission-vhost">{t("users.vhost")}</Label>
        {isPending ? (
          ...
          >
            <SelectTrigger id="topic-permission-vhost">
      ...
      <div className="space-y-2">
        <Label htmlFor="exchange">{t("users.exchangePattern")}</Label>
        <Input id="exchange" {...register("exchange")} disabled={isLoading} placeholder=".*" />
        {errors.exchange && (
          <p className="text-sm text-destructive">{errors.exchange.message as string}</p>
        )}
      </div>
      ...
      <div className="flex justify-end gap-2 pt-4">
        ...
        <Button type="submit" disabled={isLoading || isPending}>
          {isLoading ? t("common.loading") : t("users.setTopicPermission")}
        </Button>
      </div>
    </form>
  );
