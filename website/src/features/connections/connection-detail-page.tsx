...
export function ConnectionDetailPage({ name, channelsSearch }: ConnectionDetailPageProps) {
  ...
  const connectionQuery = useQuery(connectionDetailQueryOptions(context.apiClient, name, range));
  ...
  const { data: channelsData, isLoading: isLoadingChannels } = useQuery({
    ...
    queryFn: ({ signal }) => getConnectionChannels(context.apiClient, name, channelsSearch, signal),
      ...
      <DetailPageHeader
        ...
        status={
          vm ? (
            <StatusBadge
              variant={
                vm.state === "running" ? "success" : vm.state === "closed" ? "error" : "warning"
              }
            >
              {vm.state}
            </StatusBadge>
      ...
      >
                ...
                items={[
                  ...
                  ...(connection?.client_properties
                    ? [
                        {
                          label: "product",
                          value: clientPropertyText(connection.client_properties.product),
                          monospace: true,
                        },
                        {
                          label: "platform",
                          value: clientPropertyText(connection.client_properties.platform),
                          monospace: true,
                        },
                        {
                          label: "version",
                          value: clientPropertyText(connection.client_properties.version),
                          monospace: true,
                        },
                        ...clientCapabilities(connection.client_properties.capabilities),
                        {
                          label: "connection_name",
                          value: clientPropertyText(connection.client_properties.connection_name),
                          monospace: true,
                        },
                      ]
                    : []),
        ...
        {isAmqp10 ? (
          ...
          <section className="mt-8 space-y-5">
            <h2 className="px-1 text-lg font-semibold tracking-tight">{t("channels.title")}</h2>
