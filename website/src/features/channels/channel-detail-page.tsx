...
export function ChannelDetailPage({ name }: { name: string }) {
  ...
  const channel = useMemo(
    () => (query.data ? createChannelViewModel(query.data) : null),
    [query.data],
  );
        ...
        {channel ? (
          <div className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
              <MetricCard
                title={t("channels.consumers")}
                value={channel.consumerCount}
                icon={<Users aria-hidden="true" />}
              />
              <MetricCard
                title={t("channels.unacknowledged")}
                value={channel.unacknowledged}
                icon={<MessagesSquare aria-hidden="true" />}
                status={channel.unacknowledged > 0 ? "warning" : "normal"}
                statusLabel={
                  channel.unacknowledged > 0
                    ? t("channels.unacknowledgedPresent")
                    : t("channels.noUnacknowledged")
                }
              />
              <MetricCard
                title={t("channels.prefetch")}
                value={channel.prefetchCount}
                icon={<Gauge aria-hidden="true" />}
              />
              <MetricCard
                title={t("channels.publishRate")}
                value={channel.publishRate}
                unit="msg/s"
                icon={<Radio aria-hidden="true" />}
              />
              <MetricCard
                title={t("channels.deliverRate")}
                value={channel.deliverRate}
                unit="msg/s"
                icon={<Send aria-hidden="true" />}
              />
              <MetricCard
                title={t("channels.ackRate")}
                value={channel.ackRate}
                unit="msg/s"
            ...
            <SectionCard title={t("channels.properties")}>
              <DetailGrid
                ...
                items={[
                  ...
                  {
                    label: t("channels.transactional"),
                    value: channel.transactional ? t("common.yes") : t("common.no"),
                  },
                  {
                    label: t("channels.confirmMode"),
                    value: channel.confirm ? t("common.yes") : t("common.no"),
                  },
            ...
            {query.data?.pending_raft_commands != null || query.data?.cached_segments != null ? (
              <SectionCard title={t("channels.protocolDiagnostics")}>
                <DetailGrid
                  ...
                  items={[
                    {
                      label: t("channels.pendingRaftCommands"),
                      value: query.data.pending_raft_commands,
                    },
