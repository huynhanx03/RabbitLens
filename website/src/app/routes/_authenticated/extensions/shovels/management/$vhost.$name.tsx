...
export const Route = createFileRoute("/_authenticated/extensions/shovels/management/$vhost/$name")({
  component: ShovelDetailRoute,
});

export function ShovelDetailRoute() {
  const { vhost, name } = Route.useParams();
  return (
    <ExtensionRouteGuard id="shovel">
      <ShovelDetailPage vhost={vhost} name={name} />
    </ExtensionRouteGuard>
  );
}
