...
describe("JsonParameterForm", () => {
  it("uses the shared admin form surface for JSON parameters", () => {
    renderWithProviders(<JsonParameterForm vhosts={["/"]} onSubmit={vi.fn()} onCancel={vi.fn()} />);

    expect(screen.getByRole("form", { name: "JSON parameter form" })).toHaveClass("rl-admin-form");
