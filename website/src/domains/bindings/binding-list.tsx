...
export function BindingList({ vhost, resourceName, mode }: BindingListProps) {
  ...
  const isLoading = mode === "to-queue" ? queueQuery.isLoading : exchangeQuery.isLoading;

  const columns = useMemo(() => createBindingColumns(t, setBindingToDelete, mode), [t, mode]);
        ...
        description={
          bindingToDelete && (
            <>
              ...
              <strong>
                {bindingToDelete.source === "" ? "(AMQP default)" : bindingToDelete.source}
              </strong>{" "}
