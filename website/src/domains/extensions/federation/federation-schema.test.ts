...
describe("sanitizeFederationUri", () => {
  it("redacts passwords in standard amqp URIs", () => {
    expect(sanitizeFederationUri("amqp://user:secret123@host:5672/vhost")).toBe(
      "amqp://user:***@host:5672/vhost",
    );
  });

  it("redacts passwords in amqps URIs", () => {
    expect(sanitizeFederationUri("amqps://admin:password@rabbitmq.example.com/")).toBe(
      "amqps://admin:***@rabbitmq.example.com/",
    );
  });
  ...
  it("handles malformed URIs gracefully by regex matching", () => {
    ...
    expect(sanitizeFederationUri("amqp://user:pass@host1,host2")).toBe(
      "amqp://user:***@host1,host2",
    );
  });
});
