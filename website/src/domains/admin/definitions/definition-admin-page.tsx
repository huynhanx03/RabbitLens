...
export function DefinitionAdminPage() {
  ...
  const handleImport = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFileError(new Error(t("definitions.invalidFile")));
    }
  };
      ...
      <SectionCard title={t("definitions.title")}>
        <div className="grid gap-0 lg:grid-cols-2 lg:divide-x lg:divide-border/60">
          <section className="space-y-5 pb-6 lg:pb-0 lg:pr-8">
            <div>
              <h3 className="text-base font-semibold">{t("definitions.exportTitle")}</h3>
            ...
            <div className="space-y-2">
              <Label htmlFor="exportVhost">{t("definitions.vhostOptional")}</Label>
              <Select value={exportVhost} onValueChange={setExportVhost}>
                <SelectTrigger id="exportVhost" className="h-11 w-full sm:max-w-72">
              ...
              <p className="text-sm text-muted-foreground">{t("definitions.exportHint")}</p>
            </div>
          ...
          <section className="border-t pt-6 lg:border-t-0 lg:pl-8 lg:pt-0">
            <div className="mb-5">
              <h3 className="text-base font-semibold">{t("definitions.importTitle")}</h3>
              <p className="mt-1 text-sm text-muted-foreground">
            ...
            <form onSubmit={handleImport} className="space-y-5">
              ...
              <div className="space-y-2">
                <Label htmlFor="importVhost">{t("definitions.vhostOptional")}</Label>
                <Select value={importVhost} onValueChange={setImportVhost}>
                  <SelectTrigger id="importVhost" className="h-11 w-full sm:max-w-72">
                    <SelectValue placeholder={t("definitions.selectVhost")} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">{t("definitions.allVhosts")}</SelectItem>
                ...
                <p className="text-sm text-muted-foreground">{t("definitions.importHint")}</p>
              </div>
              ...
              <Button type="submit" disabled={!file || importMutation.isPending}>
                ...
                {importMutation.isPending
                  ? t("definitions.importing")
                  : t("definitions.importButton")}
              </Button>
