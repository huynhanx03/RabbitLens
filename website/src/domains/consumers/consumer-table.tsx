...
import type { Consumer } from "./consumer-schema";

/**
 * Entity presentation reused by queue and channel detail screens. It belongs
 * with the Consumer contract rather than either screen feature.
 */
      ...
      <table className="w-full min-w-5xl text-left text-sm">
        ...
        <tbody>
          {consumers.map((consumer) => {
            const owner =
              consumer.channel_details?.name ?? consumer.channel_details?.connection_name;
            return (
              <tr
                key={`${consumer.queue.vhost}-${consumer.queue.name}-${consumer.consumer_tag}`}
              ...
              >
                ...
                <td className="px-3 py-2">
                  {owner ? (
                    <a
                      ...
                      href={`/channels/${encodeURIComponent(owner)}`}
                    >
                      {owner}
                    </a>
                  ) : (
                ...
                <td className="px-3 py-2">
                  <a
                    className="text-primary hover:underline"
                    href={`/queues/${encodeURIComponent(consumer.queue.vhost)}/${encodeURIComponent(consumer.queue.name)}`}
                  >
                    {consumer.queue.name}
                  </a>
                </td>
                <td className="px-3 py-2">
                  {consumer.ack_required == null
                    ? "—"
                ...
                <td className="px-3 py-2">
                  <StatusBadge variant={consumer.active === false ? "warning" : "success"}>
                    {consumer.active === false
                      ? t("consumerDetails.inactive")
                      : t("consumerDetails.active")}
                  </StatusBadge>
                </td>
                <td className="px-3 py-2 tabular-nums">{consumer.consumer_timeout ?? "—"}</td>
                <td className="px-3 py-2">
                  <AmqpValue value={consumer.arguments ?? {}} />
                </td>
              </tr>
            );
          })}
